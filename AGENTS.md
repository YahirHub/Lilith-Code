...
- Un stream parcial no puede quedar vivo indefinidamente sólo porque el servidor siga enviando keepalives SSE: después del primer chunk semántico debe existir un watchdog de progreso real y, si se estanca, reintentar el request como una interrupción recuperable.
- Si un stream ya mostró contenido antes del corte, descartar sólo la respuesta parcial del request actual antes de reintentar; nunca duplicar texto, reasoning ni tool calls y nunca borrar el prompt del usuario. Mientras llegan deltas textuales, la TUI debe mantener un estado visible `Respondiendo…`; no dejar un turno activo con `thinking=false`, `working=false` y sin indicador de actividad.

...
func systemPrompt(activeTools []string, skillsBlock, agentsBlock, todoBlock, modeBlock string) string {
	...
	for _, rule := range []string{
		...
		"Use code_search for repository text search; it has a bounded pure-Go fallback, so do not install ripgrep solely to inspect source. run_terminal_command prefers the host shell and can fall back to shell=portable for its bounded Bash/POSIX-compatible subset; unsupported shell syntax fails explicitly, and the portable shell is neither full Bash nor a Linux userland and does not replace external git, gh, go, npm, docker or make executables.",

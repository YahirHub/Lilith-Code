...
def render(tag: str, previous_tag: str, repository: str, commits: list[Commit]) -> str:
    ...
    lines.extend(["## Instalación y actualización", "", "### Linux", "", "```bash", f"curl -fsSL https://raw.githubusercontent.com/{repository}/main/scripts/install.sh | bash", "```", "", "### Termux ARM64", "", "```bash", "pkg install -y curl", f"curl -fsSL https://raw.githubusercontent.com/{repository}/main/scripts/install.sh | sh", "```", "", "### Windows (PowerShell)", "", "```powershell", f"irm https://raw.githubusercontent.com/{repository}/main/scripts/install.ps1 | iex", "```", ""])
